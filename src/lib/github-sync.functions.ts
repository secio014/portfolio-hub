...
export const syncGithubData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: summarized } = await context.supabase
      .from("projects")
      .select("github_repo_id, summary")
      .not("github_repo_id", "is", null);
    const summarizedRepoIds = new Set(
      (summarized ?? [])
        .filter((row) => row.summary && Object.keys(row.summary as object).length > 0)
    ...
    // Auto-generate the project modal's quick summary for any repo that
    ...
    // Cached once on `projects.summary` (an override row, same as
    // featured/label overrides); never regenerated by a later sync once set,
    // and never touched by a visitor opening the modal. Admins can edit or
    // regenerate it afterwards from the admin panel. Best-effort: a
    // summarization failure must never fail the whole sync.
    ...
    for (const repo of reposNeedingSummary) {
      try {
        ...
        console.error(`[github-sync] summary generation failed for ${repo.full_name}:`, err);
      }
    }
